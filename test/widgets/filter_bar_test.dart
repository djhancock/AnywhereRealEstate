...
void main() {
  Future<void> _build(
    ...
    await tester.pumpWidget(TestRig(FilterBar<TestItem>(
      filters: filters,
      sort: sortItems,
  ...
  testWidgets("text field changes trigger the callback", (widgetTester) async {
    ...
    await _build(widgetTester, [], [], applyDelegate.applyFilter);
  ...
  testWidgets('Filter button reflects the filter criteria',
      (widgetTester) async {
    final applyDelegate = FilterActionDelegate();

    await _build(widgetTester, [filterCriteria1, filterCriteria2], [],
        applyDelegate.applyFilter);

    final filterFind = find.byWidgetPredicate(
        (widget) => widget is PopupMenuButton<FilterCriteria<TestItem>>);
    await widgetTester
        .tap(filterFind)
        .then((_) => widgetTester.pumpAndSettle(const Duration(seconds: 1)));
    ...
    expect(
        find.descendant(
            of: menuItemFind, matching: find.text(filterCriteria1.label)),
        findsOneWidget);
    expect(
        find.descendant(
            of: menuItemFind, matching: find.text(filterCriteria2.label)),
        findsOneWidget);
    expect(find.descendant(of: menuItemFind, matching: find.byType(Checkbox)),
        findsNWidgets(2));
  });
  ...
  testWidgets('Filter items reflect the selected state', (widgetTester) async {
    ...
    final menuItemFind = find.byType(PopupMenuItem<FilterCriteria<TestItem>>);
    final checkboxFind =
        find.descendant(of: menuItemFind, matching: find.byType(Checkbox));
  ...
  testWidgets('Sort button reflects the sort criteria', (widgetTester) async {
    ...
    await _build(widgetTester, [], [sortCriteria1, sortCriteria2],
        applyDelegate.applyFilter);

    final filterFind = find.byWidgetPredicate(
        (widget) => widget is PopupMenuButton<SortCriteria<TestItem>>);
    await widgetTester
        .tap(filterFind)
        .then((_) => widgetTester.pumpAndSettle(const Duration(seconds: 1)));
    expect(filterFind, findsOneWidget);
    ...
    expect(
        find.descendant(
            of: menuItemFind, matching: find.text(sortCriteria1.label)),
        findsOneWidget);
    expect(
        find.descendant(
            of: menuItemFind, matching: find.text(sortCriteria2.label)),
        findsOneWidget);
    expect(
        find.descendant(
            of: menuItemFind, matching: find.byIcon(sortCriteria1.icon)),
        findsOneWidget);
    expect(
        find.descendant(
            of: menuItemFind, matching: find.byIcon(sortCriteria2.icon)),
        findsOneWidget);
  });
  ...
  testWidgets('Sort items reflect the selected state', (widgetTester) async {
    ...
    await Future.forEach(sorts.indexed, (element) async {
      ...
      var iconFind =
          find.descendant(of: menuItemFind, matching: find.byType(Icon));

      await widgetTester
          .tap(iconFind.at(element.$1))
          .then((_) => widgetTester.pumpAndSettle(const Duration(seconds: 1)));
    });
